...
func TestUint32CursorUnmarshalerBounds(t *testing.T) {
	t.Run("maximum", func(t *testing.T) {
		// A uint64 containing math.MaxUint32.
		data := []byte{0x04, 0x02, 0xff, 0xff, 0xff, 0xff}
		var value Uint32
		_, err := mmdbdata.NewDecoder(data, 0).Cursor().UnmarshalCursor(&value)
		require.NoError(t, err)
		assert.Equal(t, Uint32(math.MaxUint32), value)
	})

	t.Run("overflow", func(t *testing.T) {
		// A uint64 containing 1 << 32.
		data := []byte{0x05, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00}
		value := Uint32(42)
		_, err := mmdbdata.NewDecoder(data, 0).Cursor().UnmarshalCursor(&value)
		require.Error(t, err)
		...
		var typeErr maxminddb.UnmarshalTypeError
		require.ErrorAs(t, err, &typeErr)
		assert.Equal(t, reflect.TypeFor[Uint32](), typeErr.Type)
	})
}
