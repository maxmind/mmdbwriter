...
func TestTopLevelMergeReturnsExistingMapWhenUnchanged(t *testing.T) {
	existing := mmdbtype.Map{
		"same":          mmdbtype.Map{"value": mmdbtype.String("unchanged")},
		"only-existing": mmdbtype.Uint32(1),
	}
	merged, err := TopLevelMerge(existing, mmdbtype.Map{
		"same": mmdbtype.Map{"value": mmdbtype.String("unchanged")},
	})
	require.NoError(t, err)

	assert.Equal(t, reflect.ValueOf(existing).Pointer(), reflect.ValueOf(merged).Pointer())
}
