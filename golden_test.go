...
func TestEnterpriseShapedOutputMatchesPreStoreGolden(t *testing.T) {
	...
	require.NoError(t, err)
	got := hex.EncodeToString(output.Bytes())
	if got != enterpriseGoldenHex {
		diff := 0
		for diff < len(got) && diff < len(enterpriseGoldenHex) &&
			got[diff] == enterpriseGoldenHex[diff] {
			diff++
		}
		if diff == len(got) || diff == len(enterpriseGoldenHex) {
			t.Fatalf("output length differs from the golden: got %d bytes, want %d bytes",
				len(got)/2, len(enterpriseGoldenHex)/2)
		}
		start := diff &^ 1
		t.Fatalf("output differs from the golden at byte offset %d: got 0x%s, want 0x%s",
			diff/2, got[start:start+2], enterpriseGoldenHex[start:start+2])
	}
}
